...
public partial class Inky : CharacterBody2D
{
    [Export]
    ...
    public override void _Ready()
    {
        ...
        if (pacmans.Count == 0)
            GD.PushError("Inky.cs: No Pacman found in `" + PacManGroup + "` group.");
        else if (pacmans.Count > 1)
            GD.PushError("Inky.cs: Found multiple Pacmans in `" + PacManGroup + "` group.");
        else
            Pacman = (Node2D)pacmans[0];

        Godot.Collections.Array<Node> maps = GetTree().GetNodesInGroup(MapGroup);
        if (maps.Count == 0)
            GD.PushError("Inky.cs: No Map found in `" + MapGroup + "` group.");
        else if (pacmans.Count > 1)
            GD.PushError("Inky.cs: Found multiple Maps in `" + MapGroup + "` group.");
        else
    ...
    private void HandleNavigation()
    {
        ...
        // TODO: change to inky navigation:
...
#pragma warning restore IDE1006 // Naming Styles
    {
        if (p_body.IsInGroup(PacManGroup))
            if (scaredTime > 0.0)
            {
                // TODO: play inky died sfx
                // TODO: play eyes returning to pen sfx
                Events.EmitInkyDied(); //  TODO: spawn eyes from current location and go to pen after that respawn inky
                QueueFree();
            }
            else
            {
