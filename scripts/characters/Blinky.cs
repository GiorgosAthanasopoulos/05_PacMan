...
public partial class Blinky : CharacterBody2D
{
    [Export]
    ...
    [Export]
    Corner ScatterCorner = Corner.TOP_LEFT;
    [Export]
    ...
    public override void _Ready()
    {
        ...
        if (pacmans.Count == 0)
            GD.PushError("Blinky.cs: No Pacman found in `" + PacManGroup + "` group.");
        else if (pacmans.Count > 1)
            GD.PushError("Blinky.cs: Found multiple Pacmans in `" + PacManGroup + "` group.");
        else
            Pacman = (Node2D)pacmans[0];

        Godot.Collections.Array<Node> maps = GetTree().GetNodesInGroup(MapGroup);
        if (maps.Count == 0)
            GD.PushError("Blinky.cs: No Map found in `" + MapGroup + "` group.");
        else if (pacmans.Count > 1)
            GD.PushError("Blinky.cs: Found multiple Maps in `" + MapGroup + "` group.");
        else
