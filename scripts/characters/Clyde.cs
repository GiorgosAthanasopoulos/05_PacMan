...
public partial class Clyde : CharacterBody2D
{
    [Export]
    ...
    public override void _Ready()
    {
        ...
        if (pacmans.Count == 0)
            GD.PushError("Clyde.cs: No Pacman found in `" + PacManGroup + "` group.");
        else if (pacmans.Count > 1)
            GD.PushError("Clyde.cs: Found multiple Pacmans in `" + PacManGroup + "` group.");
        else
            Pacman = (Node2D)pacmans[0];

        Godot.Collections.Array<Node> maps = GetTree().GetNodesInGroup(MapGroup);
        if (maps.Count == 0)
            GD.PushError("Clyde.cs: No Map found in `" + MapGroup + "` group.");
        else if (pacmans.Count > 1)
            GD.PushError("Clyde.cs: Found multiple Maps in `" + MapGroup + "` group.");
        else
    ...
    private void HandleNavigation()
    {
        ...
        // TODO: change to clyde navigation: will target Pac-Man directly, but will scatter whenever he gets within an 8 tile radius of Pac-Man.
